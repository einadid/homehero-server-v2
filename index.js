// ============================================================
// HOMEHERO BACKEND SERVER - UPDATED & FIXED CODE
// ============================================================
...
async function run() {
  try {
    ...
    // ✅ Get ALL services (no pagination) - NEW ROUTE
    app.get('/services/all', async (req, res) => {
      try {
        const services = await servicesCollection
          .find({})
          .sort({ createdAt: -1 }) // নতুন সার্ভিস আগে দেখাবে
          .toArray();
    ...
    // ✅ Get services with filtering, search, sorting & OPTIONAL pagination - FIXED
    app.get('/services', async (req, res) => {
      try {
        ...
        switch (sortBy) {
          ...
          case 'newest':
          default:
            sort = { createdAt: -1 }; // 🔥 নতুন সার্ভিস সবার আগে
        }
        ...
        // Build query
        let query = servicesCollection.find(filter).sort(sort);

        // ✅ FIXED: Pagination শুধু তখনই apply হবে যখন limit দেওয়া হবে
    ...
    app.get('/', (req, res) => {
      res.json({
        ...
        endpoints: {
          ...
          services: {
            getAll: 'GET /services (no limit = all services)',
            getAllNoPagination: 'GET /services/all',
            getWithPagination: 'GET /services?limit=12&page=1',
